...
class AutoImagePlugin(Star):
    ...
    async def _generate_and_send(
        ...
        """统一生图流程：解析参数 → 提交任务 → 发送图片。返回结果说明文本。"""
        try:
            client = self._client()
        ...
        # 参考图统一下载并转成 base64 data URL，避免 API 拉不动平台图链
            ...
            try:
                async with aiohttp.ClientSession(
                    ...
                    for src in ref_sources[:MAX_REF_IMAGES]:
                        ref_data_urls.append(await to_data_url(session, src))
            except Exception as e:
                return f"⚠️ 参考图处理失败: {e}"

        if self.config.get("show_progress_msg", True):
            try:
                await self._send(event, [Plain(
                    f"🎨 已提交生图任务（{model} / {ratio}"
                    + (f" / {size}" if size else "")
                    + (f"，参考图 {len(ref_data_urls)} 张" if ref_data_urls else "")
                    + "），生成中请稍候……"
                )])
            except Exception:
                pass
    ...
    async def tool_text_to_image(
        ...
        return await self._generate_and_send(event, prompt, params or "", [])
    ...
    async def tool_image_edit(
        ...
        return await self._generate_and_send(event, prompt, params or "", refs)
